...
### 7. Ejemplo de evolución

Una aplicación puede evolucionar desde un monolito MVC hasta una arquitectura de microservicios.

##### Etapa 1: monolito

```mermaid
flowchart LR
    Client[Cliente] --> App[Aplicación MVC]
    App --> DB[(Base de datos)]
```

##### Etapa 2: monolito con API REST
...
La evolución hacia microservicios **no es necesariamente una mejora automática**. Para aplicaciones pequeñas o medianas, un monolito bien diseñado puede ser una solución más sencilla y eficiente.
...
🔧 **A1.1.** Abre DevTools en una web que uses a diario, recarga con la caché vacía y responde:
número total de peticiones, peso transferido, dominios distintos implicados y tiempo hasta el primer
byte (TTFB).

🔧 **A1.2.** Dibuja el diagrama de capas y niveles de una aplicación que conozcas (Moodle, un
e-commerce…). Indica qué se ejecuta en cada nivel físico.

🔧 **A1.3.** Argumenta en 10 líneas si dividirías en microservicios una aplicación de gestión
académica de un centro con 800 alumnos. Justifica en términos de coste/beneficio.